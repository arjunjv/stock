...
def create_or_open_existing_table(table_name):
    
    print("create_or_open_existing_table")
    ...
    try:
        ...
        INFO("wait till table get created")
        table.wait_unitl_exist()
        INFO("table created")
...
def call_to_add_to_my_wishlist(request, session):
    table_name = "wishlist"
    table = create_or_open_existing_table(table_name)
